...
router.post('/admin/login', async (req, res) => {
    console.log('[AdminLogin] 收到管理员登录请求');
    console.log('[AdminLogin] 请求体:', req.body);
    
    const { pid, password } = req.body;
    
    if (!pid || !password) {
        console.log('[AdminLogin] 参数不完整:', { pid, hasPassword: !!password });
        return res.json({
            success: false,
            message: '请填写完整信息'
        });
    ...
    console.log('[AdminLogin] 开始管理员验证:', { pid });
    
    try {
        // 首先检查是否为管理员账号（name字段为"administrators"）
        const [results] = await pool.promise().query(
            'SELECT * FROM users WHERE pid = ? AND name = ? AND password = ?',
            [pid, 'administrators', password]
        );
        
        if (results.length === 0) {
            console.log('[AdminLogin] 管理员验证失败:', { pid });
            return res.json({
                success: false,
                message: '管理员账号或密码错误'
            });
        ...
        console.log('[AdminLogin] 管理员登录成功:', { pid, name: admin.name, level: admin.level });
        ...
        console.error('[AdminLogin] 数据库查询失败:', error);
        res.json({
            success: false,
            message: '管理员登录失败，请重试'
        });
